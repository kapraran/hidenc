...
module.exports = function(file, key) {
  ...
  if (!utils.fileExists(input)) {
    ...
    if (!utils.fileExists(input)) return utils.error('No such file "%s"', [file])
  }

  if (utils.extEquals(input, utils.ENCR_EXT)) {
    var parsed = utils.parseFile(input)
    if (parsed.length > 1) output = parsed[0]
  } else if (input != file) {
    output = file
  } else {
    var parsed = utils.parseFile(file)

    if (parsed.length > 1) output = parsed[0] + ".decr." + parsed[1]
  }

  if (!output) output = file + "." + utils.DECR_EXT
